...
class SuperpowersModeV2:
    """Superpowers 模式 v2 - 集成 LLM 决策"""
    ...
    def _llm_decide(
        ...
        Returns:
            {
                "action": "shell" | "python" | "read" | "write" | "done",
                "params": {...},
                "reasoning": "为什么这样做"
            }
        """
        ...
        if step == 1:
            return {
                "action": "shell",
                "params": {"command": "echo 'Starting task'"},
                "reasoning": "第一步：初始化任务"
            }
        elif step == 2:
            return {
...
# 便捷函数
def superpower_v2(task: str, **kwargs) -> Dict[str, Any]:
    """
    快速执行任务（Superpowers v2）
    
    Example:
        result = superpower_v2("创建一个 Flask API")
        result = superpower_v2("分析日志文件", context={"file": "app.log"})
    """
    mode = SuperpowersModeV2()
    return mode.execute(task, **kwargs)


if __name__ == "__main__":
    # 测试
    result = superpower_v2("测试 Superpowers v2")
